...
class Main {
	public static void main(String[] args) {
		...
		while (completedCount < n) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (p.at <= currentTime && p.rt > 0) {
					if (p.rt < minRemainingTime) {
						...
						if (p.at < processes[selectedIndex].at)
							selectedIndex = i;
					}
				}
			}
			...
			for (int i = 0; i < n; i++) {
				if (processes[i].at > currentTime && processes[i].at < nextArrivalTime) {
			...
			if (nextArrivalTime == Integer.MAX_VALUE) {
				...
				safeRunTime = Math.min(p.rt, nextArrivalTime - currentTime);
			}
			
			currentTime += safeRunTime;
			p.rt -= safeRunTime;
